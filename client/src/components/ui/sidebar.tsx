  ...
  return (
    ...
    >
      ...
      <div
        ...
        data-side={side}
        className={cn(
          'fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-200 ease-linear data-[side=left]:-left-2 data-[side=left]:group-data-[collapsible=offcanvas]:-left-(--sidebar-width) data-[side=right]:right-0 data-[side=right]:group-data-[collapsible=offcanvas]:-right-(--sidebar-width) md:flex',
...
>(({ render, isActive = false, variant = 'default', size = 'default', tooltip, className, ...props }, ref) => {
  ...
  const compWithRef = React.isValidElement(comp) ? React.cloneElement(comp as React.ReactElement, { ref }) : comp;
...
>(({ className, render, showOnHover = false, ...props }, ref) => {
  ...
  return React.isValidElement(comp) ? React.cloneElement(comp as React.ReactElement, { ref }) : comp;
});
